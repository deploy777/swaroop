...
window = tk.Tk()
window.title(f"KIET Chat Server - {username}")
...
tk.Label(header_left, text=f"Server Chat - {username}", bg="#1e293b", fg="#94a3b8",
...
def add_message(message, sender):
    ...
    if sender == "server":
        ...
        tk.Label(bf, text=message, bg="#16a34a", fg="white", padx=15, pady=10,
...
def send_message(event=None):
    ...
    try:
        ...
        add_message(full_msg, "server")
    except (BrokenPipeError, ConnectionResetError, OSError):
        set_disconnected()
        add_system_msg("Client disconnected")
...
def send_file():
    ...
    try:
        ...
        send_msg(conn, f"FILE:{filename}:{content}")
        add_message(f"📁 You sent: {filename}", "server")
    except (BrokenPipeError, ConnectionResetError, OSError):
        set_disconnected()
        add_system_msg("Client disconnected")
...
def send_image():
    ...
    try:
        ...
        filename = os.path.basename(file_path)
        send_msg(conn, f"IMAGE:{filename}:{encoded}")
        add_message(f"🖼️ You sent image: {filename}", "server")
    except (BrokenPipeError, ConnectionResetError, OSError):
        set_disconnected()
        add_system_msg("Client disconnected")
    except Exception as e:
...
tk.Button(bottom_frame, text="Send", command=send_message,
          bg="#16a34a", activebackground="#15803d", **btn_cfg).pack(side="right", padx=(3,10), pady=15)
tk.Button(bottom_frame, text="📁", command=send_file,
          bg="#16a34a", activebackground="#15803d", **btn_cfg).pack(side="right", padx=3, pady=15)
    ...
    while connected:
        try:
            ...
            if msg is None:
                ...
                window.after(0, add_system_msg, "Client disconnected")
                break
            ...
            if msg.startswith("IMAGE:"):
                ...
                with open(save_name, "wb") as f:
                    f.write(img_data)
                window.after(0, add_message, f"🖼️ Image received: {save_name}", "client")
                ...
                with open(fname, "w", encoding="utf-8") as f:
                    f.write(content)
                window.after(0, add_message, f"📁 Received file: {fname}", "client")

            else:
                window.after(0, add_message, msg, "client")
...
add_system_msg(f"Client connected from {addr[0]}:{addr[1]}")
